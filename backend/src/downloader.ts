  ...
  constructor(url: string, format: string, quality: string) {
    ...
    this.events = ytdlp.exec(
      [
        ...
        "res,ext:mp4:m4a",
        // '--remux-video', format,
        // https://www.reddit.com/r/youtubedl/comments/x8p2pq/whats_the_best_choice_among_hardcoding_an/
        "--remux",
        format,
        "--merge",
        format,
    ...
    this.events
      ...
      .on("error", (error: string) => {
        logger.downloader.error(
          `Failed to download ${this.url} (${this.id}): `,
          error
        );
  ...
  constructor() {
    setInterval(() => {
      this.#downloads.forEach((download) => {
        ...
        if (
          download.finished &&
          download.createdAt.getTime() + 3600000 < new Date().getTime()
        ) {
          download.cleanup();
