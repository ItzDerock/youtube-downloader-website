...
const DataUpdateContext = React.createContext((() => {}) as React.Dispatch<
  React.SetStateAction<DataContextType>
>);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      if (status.status === 500) {
        toast(
          status.data.error ?? status.data.message ?? "Internal Server Error",
          {
            type: "error",
          }
        );
